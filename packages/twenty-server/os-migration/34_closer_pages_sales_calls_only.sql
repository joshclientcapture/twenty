-- Closer pages count sales calls only, the same rule as the Sales page (os.is_sales_appointment):
-- a closer's set-up calls and the webinar seats on their calendar are not calls they can sit.
-- Generated from the live definitions of closer_call_rows and get_closer_show_up.
...
appts as (
  ...
  where b.is_new_appointment and b.host_email = c.calendly_host_email
    and os.is_sales_appointment(b.host_email, b.name, b.event_type_uri)
