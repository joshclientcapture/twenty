...
type CloserStats = {
  showUp: ShowUp | null;
  sales: CloserSales | null;
  commission: CloserCommission | null;
};
...
type CloserForm = {
  ...
  email: string;
  loginEmail: string;
  differentLogin: boolean;
  commissionRate: string;
  commissioned: boolean;
};

const EMPTY_FORM: CloserForm = {
  name: '',
  email: '',
  loginEmail: '',
  differentLogin: false,
  commissionRate: '10',
  commissioned: true,
};
...
const formFromCloser = (closer: Closer): CloserForm => {
  const workEmail =
    closer.fathomEmail ?? closer.calendlyHostEmail ?? closer.email ?? '';
  const differentLogin =
    !!closer.email && closer.email.toLowerCase() !== workEmail.toLowerCase();
  return {
    name: closer.name,
    email: workEmail,
...
const uniqueCloserId = (name: string, taken: Closer[]) => {
  ...
  let candidate = base;
  let suffix = 2;
  while (taken.some((closer) => closer.id === candidate))
...
export const ClosersPage = () => {
  ...
  const load = async () => {
    ...
    for (const closer of list.filter((c) => c.active)) {
      ...
      setStats((previous) => ({
        ...previous,
        [closer.id]: { showUp, sales, commission },
      }));
    }
  };
  useEffect(() => {
  ...
  const openNew = () => {
    setForm(EMPTY_FORM);
    setEditor({ closer: null });
  };
  const openEdit = (closer: Closer) => {
    setForm(formFromCloser(closer));
    setEditor({ closer });
  };
  const closeEditor = () => {
    setEditor(null);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const reusable = editor?.closer
        ? null
        : findReusableCloser(closers ?? [], form.name, workEmail);
      if (reusable?.active)
        throw new Error(`${reusable.name} is already a closer.`);
      await upsertCloser({
        id:
          editor?.closer?.id ??
          reusable?.id ??
          uniqueCloserId(form.name, closers ?? []),
        name: form.name.trim(),
        email:
          (form.differentLogin ? form.loginEmail.trim() : workEmail) || null,
        ...
        <StyledHeaderActions>
          ...
          {isAdmin && (
            <Button
              ...
              Icon={IconPlus}
              title="Add closer"
              onClick={() => (editor && !editing ? closeEditor() : openNew())}
            />
          )}
        </StyledHeaderActions>
      </PageHeader>
      <StyledBody>
        ...
        >
          <StyledContent>
              ...
              <StyledReveal>
                <StyledEditorCard onSubmit={submit}>
                  <div>
                    ...
                    <p>
                      Their work email is what Fathom records under and what
                      Calendly books on, so it links their calls, bookings and
                      sales automatically.
                    </p>
                  </div>
                  <StyledFields>
                    ...
                    <StyledToggle>
                      <input
                        type="checkbox"
                        checked={form.differentLogin}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            differentLogin: e.target.checked,
                          }))
                        }
                      />
                    ...
                    {form.differentLogin && (
                      <StyledFormField>
                        ...
                        <input
                          ...
                          placeholder="sam@clientcapture.io"
                          onChange={(e) =>
                            setForm((f) => ({
                              ...f,
                        ...
                        />
                        <span data-hint>
                          With a login linked they only see their own page.
                        </span>
                      </StyledFormField>
                    )}
                    <StyledToggle>
                      <input
                        type="checkbox"
                        checked={form.commissioned}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            commissioned: e.target.checked,
                          }))
                        }
                      />
                    ...
                    {form.commissioned && (
                      <StyledFormField>
                        ...
                        />
                        <span data-hint>
                          Of every payment attributed to them.
                        </span>
                      </StyledFormField>
                    )}
                  </StyledFields>
            ...
            {closers && active.length === 0 && (
              <StyledNotice>No closers yet.</StyledNotice>
            )}
            ...
            <StyledCloserGrid>
              {!closers &&
                Array.from({ length: 2 }).map((_, i) => (
                  <StyledCard
                    key={i}
                    style={{ minHeight: 160, justifyContent: 'space-between' }}
                  >
                ...
                return (
                  ...
                  >
                    ...
                    <StyledRow style={{ justifyContent: 'space-between' }}>
                      ...
                      <StyledRow>
                        <StyledChip
                          data-tone={closer.commissioned ? 'accent' : undefined}
                        >
                          {closer.commissioned
                            ? `${Math.round(closer.commissionRate * 100)}% commission`
                            : 'No commission'}
                        </StyledChip>
                        {!closer.calendlyHostEmail && (
                          <StyledChip
                            data-tone="caution"
                            title="No work email linked, so appointments stay at zero"
                          >
                            No calendar
                          </StyledChip>
                        )}
                    ...
                    <div data-stats>
                      <div>
                        <div data-k>Sales closed</div>
                        <div data-v>
                          {s?.sales ? (
                            s.sales.total.toLocaleString()
                          ) : (
                            <Skeleton width={40} />
                          )}
                        </div>
                      ...
                      <div>
                        <div data-k>Cash collected</div>
                        <div data-v>
                          {s?.commission ? (
                            fmtUsd(s.commission.revenue)
                          ) : (
                            <Skeleton width={60} />
                          )}
                        </div>
                      ...
                      <div>
                        <div data-k>Still active</div>
                        <div data-v="positive">
                          {s?.sales ? (
                            `${s.sales.active} · ${fmtUsd(s.sales.mrr)}/mo`
                          ) : (
                            <Skeleton width={70} />
                          )}
                        </div>
                      ...
                      {closer.commissioned && (
                        <div>
                          <div data-k>Commission outstanding</div>
                          <div
                            data-v={
                              s?.commission && s.commission.outstanding > 0
                                ? 'caution'
                                : 'positive'
                            }
                          >
                            {s?.commission ? (
                              fmtUsd2(s.commission.outstanding)
                            ) : (
                              <Skeleton width={60} />
                            )}
                          </div>
