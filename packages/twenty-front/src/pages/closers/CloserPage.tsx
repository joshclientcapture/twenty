...
const CallEditor = ({ row, busy, closerName, onSave, onReset, onClose }: { row: LogRow; busy: boolean; closerName: string; onSave: (p: CallPatch) => void; onReset: () => void; onClose: () => void }) => {
      ...
      <StyledRow>
        ...
        {trial && (
          <StyledField>
            ...
            <StyledMuted>attributes the trial + sale to {firstName(closerName)}</StyledMuted>
          </StyledField>
        )}
      </StyledRow>
      <StyledRow>
...
export const CloserPage = ({ closerId: closerIdProp }: CloserPageProps) => {
  ...
  const restricted = !isAdmin && ownCloser !== null && ownCloser.id !== closerId;

  useEffect(() => {
    fetchClosers().then(setClosers).catch(() => setClosers([]));
  }, []);

  const loadComm = () => fetchCloserCommission(closerId).then(setComm).catch(() => {});
  useEffect(() => {
    if (!closer || restricted) return;
    let cancelled = false;
    setSu(null); setConv(null); setSales(null); setCash(null); setComm(null);
    fetchCloserShowUp(closerId).then((d) => !cancelled && setSu(d)).catch(() => {});
    fetchCloserConversion(closerId).then((d) => !cancelled && setConv(d)).catch(() => {});
  ...
  const loadDaily = (from: string, to: string) => {
    ...
    return fetchCloserDaily(closerId, from, to).then(setDaily).catch(() => setDaily(null)).finally(() => setDailyLoading(false));
  };
  useEffect(() => {
    if (!closer || restricted) return;
  ...
  const saveCall = async (key: string, patch: CallPatch) => {
    setSavingKey(key);
    setErr(null);
    try { await setCloserCall(closerId, { key, ...patch }); await loadDaily(dayRange.from, dayRange.to); setEditKey(null); }
  ...
  const resetCall = async (key: string) => {
    setSavingKey(key);
    setErr(null);
    try { await clearCloserCall(closerId, key); await loadDaily(dayRange.from, dayRange.to); setEditKey(null); }
  ...
  if (closers && !closer) {
    return (
      <StyledPage>
        ...
        <StyledBody><StyledContent>
          <StyledNotice>No closer with id "{closerId}". <StyledTextLink to="/closers">Back to closers</StyledTextLink></StyledNotice>
        </StyledContent></StyledBody>
      </StyledPage>
    );
  }
  if (closer && restricted) {
    return (
      <StyledPage>
        ...
        <StyledBody><StyledContent>
          <StyledNotice>You can only view your own page. <StyledTextLink to={`/closers/${ownCloser!.id}`}>Open {ownCloser!.name}</StyledTextLink></StyledNotice>
        </StyledContent></StyledBody>
      </StyledPage>
    );
  }
  ...
  const salesSeries = monthsAsc.map((r) => ({ label: moLabel(r.month_key), value: sales?.monthly[r.month_key]?.total ?? 0, sublabel: moLabel(r.month_key) }));
        ...
        <SkeletonTheme baseColor={'var(--t-background-tertiary)'} highlightColor={'var(--t-background-transparent-lighter)'} borderRadius={4}>
          <StyledContent>
            ...
            {closer && !closer.calendlyHostEmail && (
              <StyledNotice>No Calendly host is linked to {firstName(closer.name)} yet, so appointment counts stay at zero. Fathom recordings still show in the call log.</StyledNotice>
            )}
              ...
              <StyledCard>
                <StyledCardHead>
                  <div>
                    ...
                    <StyledBigValue>
                      {sales ? salesHis.toLocaleString() : <Skeleton width={60} />}
            ...
            <StyledCard>
              <StyledCardHead>
                ...
                <StyledCardHint title={`Counts new appointments only (Discovery, Demo, Agency Demo). Next Steps follow-ups are separate. Not counted: ${resc} rescheduled (moved to a new time, counted once) · ${can} cancelled.`}>
                        ...
                        <Fragment key={r.key || i}>
                          ...
                          {open && (
                            <tr data-subrow>
                              <td colSpan={5}>
                                <StyledReveal>
                                  <CallEditor row={r} busy={savingKey === r.key} closerName={name} onSave={(p) => saveCall(r.key, p)} onReset={() => resetCall(r.key)} onClose={() => setEditKey(null)} />
                                </StyledReveal>
                              </td>
                            </tr>
                          )}
            ...
            <StyledCard>
              <StyledCardHead>
                ...
                <StyledCardHint>{commissionPct}% of every attributed payment · next payout {comm ? fmtDate(comm.next_payout) : '—'}</StyledCardHint>
              </StyledCardHead>
              {comm && !comm.commissioned ? (
                <StyledMuted>{firstName(name)} is not on commission. {comm.payments} attributed payments · {fmtUsd2(comm.revenue)} revenue.</StyledMuted>
              ) : comm ? (
                <>
                  <StyledCommRow>
                    <StyledMuted>{comm.payments} payments · {fmtUsd2(comm.revenue)} attributed revenue</StyledMuted>
