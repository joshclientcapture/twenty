...
const CONFIG: Record<
  string,
  { title: string; sub: string; cols: Col[]; back: string }
> = {
  calls: {
    title: 'Appointments sat',
    sub: 'New appointments attended (Fathom recording matched a held booking) — excludes Next Steps follow-ups & internal meetings',
    back: '/closers',
    cols: [
  ...
  cancellations: {
    title: 'Cancellations',
    sub: 'Genuine cancellations only — reschedules are excluded (they land on the new date)',
    back: '/closers',
    cols: [
      { key: 'name', header: 'Prospect', kind: 'text' },
      { key: 'email', header: 'Email', kind: 'text' },
  ...
  noshows: {
    title: 'No-shows',
    sub: 'Held appointments (not cancelled, not rescheduled) with no Fathom recording',
    back: '/closers',
    cols: [
      { key: 'name', header: 'Prospect', kind: 'text' },
      { key: 'email', header: 'Email', kind: 'text' },
  ...
  trials: {
    ...
    sub: 'From the Conversifi database',
    back: '/sales',
    cols: [
      { key: 'name', header: 'Customer', kind: 'text' },
      { key: 'email', header: 'Email', kind: 'text' },
  ...
  therapon_customers: {
    title: 'Therapon — customers',
    sub: "Every customer he closed · filter by status or plan to see who's still active",
    back: '/closers',
    cols: [
      { key: 'name', header: 'Customer', kind: 'text' },
      { key: 'email', header: 'Email', kind: 'text' },
  ...
  closer_customers: {
    ...
    sub: "Every customer this closer closed · filter by status or plan to see who's still active",
    back: '/closers',
    cols: [
      { key: 'name', header: 'Customer', kind: 'text' },
      { key: 'email', header: 'Email', kind: 'text' },
  ...
  sales: {
    ...
    sub: 'Every payment attributed to this closer · repeat payments marked · click a name for full history',
    back: '/closers',
    cols: [
      { key: 'name', header: 'Customer', kind: 'text' },
      { key: 'email', header: 'Email', kind: 'text' },
  ...
  sales_comm_due: {
    ...
    sub: "Payments whose 10% commission hasn't been paid yet · mark each once settled",
    cols: commCols,
    back: '/closers',
  },
  sales_comm_paid: {
    title: 'Commission — paid',
    sub: 'Payments whose 10% commission has been paid out',
    cols: commCols,
    back: '/closers',
  },
  customer_payments: {
    title: 'Customer payments',
    sub: 'Every payment from this customer',
    back: '/closers',
    cols: [
  ...
  new_customers: {
    title: 'New customers',
    sub: 'Customers who started paying in this period',
    back: '/sales',
    cols: [
      { key: 'name', header: 'Customer', kind: 'text' },
      { key: 'email', header: 'Email', kind: 'text' },
  ...
  churned_customers: {
    ...
    sub: 'Customers who fully cancelled in this period — no active subscription remaining',
    back: '/sales',
    cols: [
      { key: 'name', header: 'Customer', kind: 'text' },
      { key: 'email', header: 'Email', kind: 'text' },
  ...
  setters_added: {
    title: 'Setters added',
    sub: 'New appointment setters by month',
    cols: setterCols,
    back: '/sales',
  },
  setters_idle: {
    title: 'Idle setters',
    sub: 'Connected but not in an active campaign',
    cols: setterCols,
    back: '/sales',
  },
  setters_active: {
    title: 'Active setters',
    sub: 'In an active campaign',
    cols: setterCols,
    back: '/sales',
  },
  setters_disconnected: {
    title: 'Disconnected setters',
    sub: 'Need reconnect',
    cols: setterCols,
    back: '/sales',
  },
  setters_eligible: {
    title: 'Bonus-eligible setters',
    sub: '30+ days active — recruiter bonus earned',
    cols: setterCols,
    back: '/sales',
  },
  setters_bonus_due: {
    title: 'Setter bonuses — due',
    sub: "Eligible 30+ day setters whose $25 bonus hasn't been paid yet — check tenure & campaign, then mark paid",
    cols: setterBonusCols,
    back: '/sales',
  },
  setters_bonus_paid: {
    title: 'Setter bonuses — paid',
    sub: 'Eligible setters whose $25 bonus has already been paid out',
    cols: setterBonusCols,
    back: '/sales',
  },
  setters_pending: {
    title: 'Pending setters',
    sub: 'Active, still under 30 days',
    cols: setterCols,
    back: '/sales',
  },
  setters_all: {
    title: 'All setters',
    sub: 'Tenure & 30-day bonus eligibility',
    cols: setterCols,
    back: '/sales',
  },
  setters_archived: {
    title: 'Archived accounts',
    sub: 'Archived copies — excluded from setter metrics & bonuses',
    cols: setterCols,
    back: '/sales',
  },
};
...
const sortValue = (v: RecordRow[string], kind?: string): number | string => {
  if (v == null || v === '')
    return kind && (NUMERIC_KINDS.has(kind) || DATE_KINDS.has(kind))
      ? -Infinity
      : '';
...
const Cell = ({ col, value }: { col: Col; value: RecordRow[string] }) => {
  ...
  switch (col.kind) {
    case 'datetime':
      return <span>{fmtDateTime(String(value))}</span>;
    case 'date':
      return <span>{fmtDate(String(value))}</span>;
    case 'money':
      return <span style={{ fontWeight: 500 }}>{fmtUsd(Number(value))}</span>;
    case 'bool':
      return value ? (
        <StyledPositive>Yes</StyledPositive>
      ) : (
        <StyledMuted>No</StyledMuted>
      );
    case 'recording':
      return (
        <StyledExtLink href={String(value)} target="_blank" rel="noreferrer">
          ▶ Watch
        </StyledExtLink>
      );
    case 'days':
      return <span>{Number(value)} days</span>;
    case 'payment': {
      const n = Number(value);
      const ord =
        n === 1 ? '1st' : n === 2 ? '2nd' : n === 3 ? '3rd' : `${n}th`;
      return n > 1 ? (
        <StyledPositive>{ord} · repeat</StyledPositive>
      ) : (
        <StyledMuted>{ord} · new</StyledMuted>
      );
    }
    case 'eligibility': {
      const v = String(value);
      if (v === 'eligible')
        return <StyledChip data-tone="positive">Eligible</StyledChip>;
      if (v === 'paused')
        return <StyledChip data-tone="caution">Paused</StyledChip>;
      return <StyledChip>Pending</StyledChip>;
    }
    case 'campaign':
      return String(value) === 'In campaign' ? (
        <StyledChip data-tone="positive">● In campaign</StyledChip>
      ) : (
        <StyledChip>○ Dormant</StyledChip>
      );
    case 'requests': {
      const n = Number(value);
      return n === 0 ? (
        <StyledCaution>0 · none sent</StyledCaution>
      ) : (
        <span>
          {n.toLocaleString()} <StyledMuted>reqs</StyledMuted>
        </span>
      );
    }
    default:
...
const SortMark = ({ on, dir }: { on: boolean; dir: 'asc' | 'desc' }) => (
  <span data-sort={on ? 'on' : undefined}>
    {on ? (dir === 'asc' ? '▲' : '▼') : '↕'}
  </span>
);
...
const RecordsPageContent = () => {
  ...
  const isComm =
    type === 'sales' || type === 'sales_comm_due' || type === 'sales_comm_paid';
  ...
  const load = () =>
    fetchRecords(type, month, arg, from, to)
      .then(setRows)
      .catch(() => setRows([]));
  useEffect(() => {
    setRows(null);
  ...
  const reattr = async (accountId: string, recruiter: string) => {
    setBusy(accountId);
    try {
      await setSetterRecruiter(accountId, recruiter);
      await load();
    } finally {
      setBusy(null);
    }
  ...
  const toggleBonus = async (accountId: string, paid: boolean) => {
    setBusy(accountId);
    try {
      await markSetterBonus(accountId, paid);
      await load();
    } finally {
      setBusy(null);
    }
  ...
  const toggleComm = async (paymentId: string, paid: boolean) => {
    setBusy(paymentId);
    try {
      await markCommissionPaid(paymentId, paid);
      await load();
    } finally {
      setBusy(null);
    }
  ...
  const monthLabel =
    from || to
      ...
      : month
        ...
        : 'All time';
  const pageTitle =
    type === 'customer_payments' && nameParam
      ? `Payments — ${nameParam}`
      : nameParam && (from || to)
        ? `${cfg.title} — ${nameParam}`
        : cfg.title;
  ...
  const monthsAvail = useMemo(
    () =>
      Array.from(
        new Set(
          (rows ?? [])
            .map((r) => (r.date ? String(r.date).slice(0, 7) : ''))
            .filter(Boolean),
        ),
      )
        .sort()
  ...
  const sortKind = useMemo(
    () => cfg.cols.find((c) => c.key === sortKey)?.kind,
    [cfg, sortKey],
  );
  const toggleSort = (key: string, kind?: string) => {
    if (sortKey === key) setSortDir((d) => (d === 'asc' ? 'desc' : 'asc'));
      ...
      setSortKey(key);
      setSortDir(
        kind && (NUMERIC_KINDS.has(kind) || DATE_KINDS.has(kind))
          ? 'desc'
          : 'asc',
      );
    }
  ...
  const visible = useMemo(() => {
    ...
    if (monthF !== 'all')
      r = r.filter((x) => x.date && String(x.date).slice(0, 7) === monthF);
    if (sortKey) {
      r = [...r].sort((a, b) => {
  ...
  const avgRequests = useMemo(() => {
    ...
    return Math.round(
      withReq.reduce((s, r) => s + Number(r.requests_sent || 0), 0) /
        withReq.length,
    );
  }, [visible]);
      ...
      <StyledBody>
        <StyledContent>
          <StyledRow style={{ justifyContent: 'space-between' }}>
            <StyledLabel>
              {cfg.sub} ·{' '}
              <span style={{ color: 'var(--t-font-color-primary)' }}>
                {monthLabel}
              </span>
            </StyledLabel>
            <StyledRow>
                ...
                <StyledField>
                  <OsSelect
                    ...
                    options={[
                      ...
                      ...monthsAvail.map((mk) => ({
                        value: mk,
                        label: new Date(mk + '-01').toLocaleDateString(
                          'en-GB',
              ...
              <StyledTable>
                <thead>
                  <tr>
                    {cfg.cols.map((c) => (
                      <th key={c.key}>
                        <button onClick={() => toggleSort(c.key, c.kind)}>
                          {c.header}{' '}
                          <SortMark on={sortKey === c.key} dir={sortDir} />
                        </button>
                      </th>
                    ))}
                    {isSetter && (
                      <th>
                        <button onClick={() => toggleSort('recruiter')}>
                          Recruiter{' '}
                          <SortMark
                            on={sortKey === 'recruiter'}
                            dir={sortDir}
                          />
                        </button>
                      </th>
                    ...
                    {isSetter && (
                      <th>
                        <button onClick={() => toggleSort('bonus_paid')}>
                          Bonus paid{' '}
                          <SortMark
                            on={sortKey === 'bonus_paid'}
                            dir={sortDir}
                          />
                        </button>
                      </th>
                  ...
                  {rows !== null && visible.length === 0 && (
                    <tr>
                      <td
                        ...
                        style={{ textAlign: 'center', padding: 40 }}
                      >
                        <StyledMuted>No records.</StyledMuted>
                      </td>
                    </tr>
                  )}
                  {visible.map((r, i) => (
                    <tr key={i}>
                        ...
                        <td key={c.key}>
                          {c.key === 'name' ? (
                            isComm && r.customer_id ? (
                              <StyledTextLink
                                to={`/records?type=customer_payments&arg=${encodeURIComponent(String(r.customer_id))}&name=${encodeURIComponent(String(r.name ?? ''))}`}
                              >
                            ...
                            String(r.status) === 'disconnected' ? (
                            <StyledCaution title="Day count is paused while the account is disconnected">
                              {Number(r[c.key])} days · paused
                            </StyledCaution>
                          ) : (
                      ...
                      {isSetter && (
                        <td>
                          <StyledField>
                            <OsSelect
                              id={`recruiter-${r.account_id}`}
                              value={String(r.recruiter ?? 'Jade Vieira')}
                              disabled={busy === r.account_id}
                              onChange={(value) =>
                                reattr(String(r.account_id), value)
                              }
                              options={RECRUITERS.map((rc) => ({
                                value: rc,
                                label: rc,
                              }))}
                            />
                        ...
                        <td>
                          {r.bonus_paid ? (
                            <Button
                              ...
                              disabled={busy === r.account_id}
                              onClick={() =>
                                toggleBonus(String(r.account_id), false)
                              }
                            />
                            ...
                            <Button
                              ...
                              disabled={busy === r.account_id}
                              onClick={() =>
                                toggleBonus(String(r.account_id), true)
                              }
                            />
                          ) : (
                        ...
                        <td>
                          {r.commission_paid ? (
                            <Button
                              ...
                              onClick={() =>
                                toggleComm(String(r.payment_id), false)
                              }
                            />
                          ) : (
                            <Button
                              ...
                              disabled={busy === r.payment_id}
                              onClick={() =>
                                toggleComm(String(r.payment_id), true)
                              }
                            />
                          )}
                        </td>
